...
import { Map, Loader2, ImageIcon } from 'lucide-react';
...
const MindMapper = () => {
  ...
  const handleGenerateText = async () => {
    ...
    try {
      ...
      setTextResult(result.candidates?.[0]?.content?.parts?.[0]?.text || 'Metin oluşturulamadı.');
    } catch (err) {
  ...
  const handleVisualize = async () => {
    ...
    try {
      ...
      setError(`Resim oluşturulurken hata: ${err.message}`);
    } finally {
